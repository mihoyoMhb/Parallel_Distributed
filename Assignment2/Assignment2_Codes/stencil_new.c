...
int main(int argc, char **argv) {
    ...
    // 8. 将全局数据分发至各进程，将有效数据放在 buf_current[EXTENT, EXTENT+local_N)
    MPI_Scatterv(
    ...
    // 10. 定义有效数据在缓冲区中的下标范围
    // 有效数据区在 buf_current[eff_start, eff_end)
    int eff_start = EXTENT;
    int eff_end   = EXTENT + local_N;  // 注意：有效区长度 = local_N
    ...
    for (int step = 0; step < num_steps; step++) {
        ...
        // 与通信重叠，先计算安全区域
        // 安全区域：不依赖最新 ghost 数据的部分，即有效区内下标 [eff_start+EXTENT, eff_end-EXTENT)
        ...
        // 计算左边界区域：有效区下标 [eff_start, eff_start+EXTENT)
        ...
        // 计算右边界区域：有效区下标 [eff_end-EXTENT, eff_end)
        ...
        // 利用双缓冲策略，交换 buf_current 与 buf_next 指针，无需 memcpy
        double *tmp = buf_current;
        buf_current = buf_next;
        buf_next = tmp;
    }

    // 13. 收集最终结果：有效数据存储在 buf_current[eff_start, eff_end)
